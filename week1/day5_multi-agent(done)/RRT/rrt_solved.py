...
class RRT:
    ...
    def steer(self, from_node, to_point):
        ...
        new_pos = (from_node.pos[0] + direction[0] * self.step_size, from_node.pos[1] + direction[1] * self.step_size)
    ...
    def find_nearby_nodes(self, new_node):
        ...
        return [node for node in self.tree if utils.euclidean_distance(node.pos, new_node.pos) < self.neigh_radius]
    ...
    def rewire(self, new_node, nearby_nodes):
        ...
        for node in nearby_nodes:
            ...
            potential_cost = new_node.cost + utils.euclidean_distance(new_node.pos, node.pos)
    ...
    def expand_tree(self):
        ...
        rand_point = (random.uniform(0, self.width - 1), random.uniform(0, self.height - 1))

        # gasirea nodului din arborele curent care este cel mai apropiat de pozitia aleasa
        nearest_node = min(self.tree, key=lambda node: utils.euclidean_distance(node.pos, rand_point))
        ...
        if self.is_collision_free(nearest_node, new_node):
            ...
            if self.RRT_Star:
                ...
                if nearby_nodes:
                    # se determina nodul care daca ar fi parintele nodului curent ar da un cost minim
                    min_cost_node = min(nearby_nodes,
                                        key=lambda node: node.cost + utils.euclidean_distance(node.pos, new_node.pos))
                    

                    # se calculeaza costul potential
                    potential_cost = min_cost_node.cost + utils.euclidean_distance(min_cost_node.pos, new_node.pos)
